...
const remisionTonerService = require('../services/remisionToner.service')
...
router.get('/total', async(req, res) => {
  try {
    const remisiones = await remisionTonerService.obtenerTodasRemisiones()
...
router.post('/', async (req, res) => {
    try {
        const nuevaRemision = await remisionTonerService.crearRemision(req.body)
...
router.patch('/:numero_remision/cancelar', async (req, res) => {
  try {
    ...
    const resultado = await remisionTonerService.cancelarRemision(numero_remision, usuarioCancelacion)
...
router.put('/:numero_remision/confirmar', async (req, res) => {
    try {
      const resultado = await remisionTonerService.confirmarEntregaRemision(req.params.numero_remision);
      res.json(resultado);
    } catch (error) {
      next(error)
    }
...
router.get('/', async (req, res, next) => {
    try {
      ...
      const remisiones = await remisionTonerService.buscarRemisiones(filtros);
      res.json(remisiones);
    } catch (error) {
      next(error)
    }
...
router.get("/:numero_remision", async (req, res) => {
  try {
      ...
      const remision = await remisionTonerService.obtenerRemisionPorNumero(numero_remision);
...
router.post('/:numero_remision/evidencia', uploadEvidencia.single('archivo'), async (req, res) => {
  try {
    ...
    const remisionActualizada = await remisionTonerService.subirEvidencia(numero_remision, nombreArchivo)
...
router.patch('/:numero_remision/fecha-programada', async (req, res) => {
  try {
    ...
    const resultado = await remisionTonerService.modificarFechaProgramada(numero_remision, nuevaFecha)
